...
rmse(prf, Data0$Net_demand[sel_b]) ## Random Forest (mtry=7): RMSE ~ 1353
pinball_loss(Data0$Net_demand[sel_b], prf, 0.8) ## Random Forest: pinball@0.8 ~ 562
...
# FIX: evaluate the GAM+RF (residuals) model on its OWN prediction (final_pred),
# not on gam_ridge.forecast (a different model). The previous code was a
# copy-paste bug that reported the ridge-GAM pinball under the GAM+RF heading.
pinball_loss(Data0$Net_demand[sel_b], final_pred, 0.8)
...
### scaling columns — FIX: use TRAIN-ONLY statistics (sel_a) to standardise.
### The previous version used full-sample mean/sd (including the validation
### rows sel_b), i.e. transductive leakage that makes the validation numbers
### mildly optimistic. final_submission.R already standardises train-only
### (mu_tr/sd_tr); we match it here so the Rmd validation is leakage-free too.
mu_tr <- colMeans(X[sel_a, , drop = FALSE])
sd_tr <- apply(X[sel_a, , drop = FALSE], 2, sd)
for (j in 1:ncol(X)){
X[,j] <- (X[,j]-mu_tr[j]) / sd_tr[j]}
X <- cbind(X,1)
d <- ncol(X)
y <- Data0$Net_demand
